...
public class XmlIO implements FileIO {
    ...
    @Override
    public <T> void write(final File file, final Class<T> type, final T t, final String localPart) throws IOException {
        try {
            final JAXBContext jaxbContext = JAXBContext.newInstance(type);
            final Marshaller marshaller = jaxbContext.createMarshaller();
            final JAXBElement<T> jaxbElement = new JAXBElement<>(new QName(localPart), type, t);
